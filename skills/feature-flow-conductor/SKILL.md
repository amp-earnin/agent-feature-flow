...
- **Deterministic selection recognition (author-bound).** On a later cycle, a reply is recognized as a selection for an awaiting-choice entry **only when both** hold: (1) the reply **routes to that entry** by one of the two paths in the routing rule above — **either** a linkage match (its `in_reply_to_id` / review-thread linkage matches the entry's `comment_id`, on the inline review surface) **or**, on the issue-comment surface that carries no linkage, an author match (its author login equals the entry's `commenter_login`, binding to the most-recent open entry for that login) — **and** (2) **exactly one valid token** (a token matching the entry's `reply_token_prefix` and one of its `options`) is present in the reply. The author-match path leans on author binding for its safety: with no linkage to disambiguate, the bound `commenter_login` plus the single-token requirement is what distinguishes a real selection from unrelated chatter.
  - **Author binding (anti-spoofing).** The reply's author login **MUST equal the entry's `commenter_login`** — on **both** routing paths, and it is the **sole** disambiguator on the no-linkage issue-comment path. A token from **any other login is ignored** — treated as "discuss", never applied. On a public delivery PR a third party can post `option: A`, but only the original commenter's selection unlocks a fix + push. This mirrors the `bot_identity` defense the convergence loop already uses against spoofed replies; the selection protocol needs the analogous binding because the apply-fix step pushes to the delivery branch. (A configured allowlist could widen "who may select" later, but the default and the safe floor is the original commenter only.)
