...
**Route replies to open awaiting-choice entries first (before dedupe / re-prompt).** A selection reply is itself a new comment with a fresh `comment_id`, so it would otherwise pass dedupe as "genuinely new" and get a brand-new multiple-choice prompt instead of being recognized as a selection. Before treating any surviving comment as a new comment needing its own prompt, check whether it is a **reply that links to an open `review_loop.monitoring.awaiting_choice` entry** — match the comment's `in_reply_to_id` (inline replies carry the parent comment id) or its review-thread linkage against the `comment_id` of each open entry. If it links to an open entry, **route it to selection-recognition** (the protocol below) for that entry instead of re-prompting; do not treat it as a fresh top-level comment.
