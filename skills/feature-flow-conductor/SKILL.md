...
- **Author + token match (issue-comment surface — no linkage available).** GitHub **issue comments have no `in_reply_to_id` and no threaded-reply linkage** (that field exists only on inline review comments), so a selection posted as a top-level issue comment can never satisfy the linkage path. For a surviving comment that carries **no** linkage, route it to an open entry when its **author login equals that entry's `commenter_login`** (the bound author recorded when the prompt was posted) **and** that author has **exactly one** open `awaiting_choice` entry **and** the comment carries **exactly one valid token** under that entry's `reply_token_prefix`. If the bound author has **more than one** open entry, the match is **ambiguous** — option tokens (`A`, `B`, …) are shared across entries, so a bare token does not identify which entry the author meant. **Never** bind by recency; instead treat such a comment as **"discuss"** and route it to selection-recognition as ambiguous (the protocol below re-prompts the author to disambiguate — e.g. by replying on the specific inline thread or naming the item — and applies **no** fix). A no-linkage comment that matches no open `commenter_login`, or carries no valid token, is **not** a selection — it falls through to the normal new-comment path.
...
**Dedupe** the remaining (non-reply, top-level) human comments: drop any whose `comment_id` is **either** already recorded in `review_loop.monitoring.handled_comment_ids` **or** already tracked by an open `awaiting_choice` entry (i.e. a prompt was already posted for it and it is still awaiting a selection). The second clause closes a double-post bug: an awaiting (not-yet-answered) comment's `comment_id` is not added to `handled_comment_ids` until its fix lands, so without it the same prompt would be re-posted on every poll. (The edited-comment re-open rule below still fires for these via content hash / `updated_at`, so a genuinely edited comment is re-evaluated rather than starved.) **Process the remaining new comments in `created_at` order** (oldest first) — never coalesce multiple comments into one reply; each new comment gets its own analysis and reply.
...
- **Deterministic selection recognition (author-bound).** On a later cycle, a reply is recognized as a selection for an awaiting-choice entry **only when both** hold: (1) the reply **routes to that entry** by one of the two paths in the routing rule above — **either** a linkage match (its `in_reply_to_id` / review-thread linkage matches the entry's `comment_id`, on the inline review surface) **or**, on the issue-comment surface that carries no linkage, an author match (its author login equals the entry's `commenter_login`) that is **unambiguous** — i.e. that author has **exactly one** open `awaiting_choice` entry; if the author has more than one open entry the bare token is ambiguous and is treated as "discuss" rather than bound to any entry (**never** to the most-recent) — **and** (2) **exactly one valid token** (a token matching the entry's `reply_token_prefix` and one of its `options`) is present in the reply. The author-match path leans on author binding for its safety: with no linkage to disambiguate, the bound `commenter_login` plus the single-open-entry and single-token requirements are what distinguish a real selection from unrelated chatter — and when the author has multiple open entries, no fix is applied on a bare token.
  - **Author binding (anti-spoofing).** The reply's author login **MUST equal the entry's `commenter_login`** — on **both** routing paths. A token from **any other login is ignored** — treated as "discuss", never applied. On a public delivery PR a third party can post `option: A`, but only the original commenter's selection unlocks a fix + push. This mirrors the `bot_identity` defense the convergence loop already uses against spoofed replies; the selection protocol needs the analogous binding because the apply-fix step pushes to the delivery branch. (A configured allowlist could widen "who may select" later, but the default and the safe floor is the original commenter only.) **Author binding is necessary but not sufficient on the no-linkage issue-comment path:** because option tokens are shared across entries, the bound author must also have **exactly one** open `awaiting_choice` entry for a bare token to identify a selection. If the bound author has **more than one** open entry, the token is ambiguous and is treated as "discuss" (never bound to the most-recent entry) — the disambiguator on that path is the single-open-entry condition, not the author login alone.
  - **Ambiguous** (two or more valid tokens present) **or absent** (no valid token present) ⇒ treat the reply as **"discuss"**: re-prompt (reply with clarification / the options again), and **never** fall back to a default choice. **No fix is applied without an unambiguous single-token match from the bound author.**
  - Only an exact single-token match **from `commenter_login`** counts as a selection and unlocks the apply-fix step below.
...
When the **bound author** (the entry's `commenter_login`) makes an **unambiguous selection** (a linked reply carrying exactly one valid token, per the author-bound protocol above) for an `awaiting_choice` entry:
