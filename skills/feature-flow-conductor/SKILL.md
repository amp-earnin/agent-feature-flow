...
- Compute `live_hash` = the content hash of the fetched comment's current body (hash the body **after** the nonce-fence/sanitize step above so the comparison is over the same normalized text both times).
...
- **Capture the edit-detection baseline at prompt time.** When the prompt is posted, compute the **content hash of the original comment's body** (hashed over the nonce-fenced/sanitized text, the same normalization the edit-detection step uses) and store it as the entry's `content_hash`. This is the baseline the edited-comment re-open rule compares the live body against on a later cycle; capturing it **here, when the prompt is posted**, is what guarantees the comparison always has a stored left-hand side. The same hash feeds the `reply_token_prefix` derivation above.
...
5. **Record handled + resolved in the workspace.** After the fix lands (push succeeds), transition the comment from awaiting to handled **atomically** — the two halves below are one state move that preserves the **handled-XOR-awaiting invariant** (the comment leaves the awaiting store in the same step it enters the handled stores, so it is never in both): record the human comment's `comment_id` in `review_loop.monitoring.handled_comment_ids` **and** record its **edit-detection baseline** — store the comment body's content hash under that `comment_id` in `review_loop.monitoring.handled_comment_hashes` (carry over the entry's just-resolved `content_hash`, or re-hash the current body), so a later edit of this now-handled comment can be detected by the edited-comment re-open rule — **and remove its `awaiting_choice` entry** (this is the awaiting-side half of the same transition; the comment is now tracked **only** by the handled stores). Then **mark the comment resolved in the workspace records** — the stacked-mode analogue of a thread resolve, written to `<WS>/` (e.g. the triage/monitoring record), **never** as a mutation on the target PR — exactly as the existing stacked fix-subagent marks findings resolved in `<WS>/triage.json` rather than on the target. Update `review_loop.monitoring.last_activity_at` to reflect that a human comment was just handled (this also keeps the idle tracking correct across cycles). (The reverse transition — handled back to awaiting when a handled comment is later edited — is the edited-comment re-open rule's case (a), which clears both handled stores before opening a fresh awaiting entry, the exact mirror of this step.)
