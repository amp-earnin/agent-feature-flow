...
- Look up the **stored baseline** for that comment: for an **open awaiting entry**, its `content_hash` (recorded when the prompt was posted); for a **handled comment**, the entry for its `comment_id` in `handled_comment_hashes` (recorded when the comment was handled).
...
If it was edited, **re-open it as unselected** — treat it as a new human comment for this cycle (re-analyze and re-prompt) rather than silently skipping it on the strength of its ID. For an open awaiting entry, **replace its prompt with a fresh one**: capture the new body's hash as the new `content_hash`, which (per Task 12) derives a `reply_token_prefix` **distinct from the original** (the prefix folds in `content_hash`, so a changed body yields a changed token) — the stale original token can no longer satisfy the re-opened entry, forcing the author to re-select. If `live_hash` equals the stored baseline, the comment is unchanged and the dedupe drop stands (no re-open). This re-open path and the dedupe drop are **mutually exclusive**: the re-open takes precedence on an edit, the drop holds otherwise, so an edit is never ignored just because the ID was seen before — whether the comment was already handled or is still awaiting a selection — and an unedited comment is never re-posted.
...
5. **Record handled + resolved in the workspace.** After the fix lands (push succeeds), record the human comment's `comment_id` in `review_loop.monitoring.handled_comment_ids` **and** record its **edit-detection baseline** — store the comment body's content hash under that `comment_id` in `review_loop.monitoring.handled_comment_hashes` (carry over the entry's just-resolved `content_hash`, or re-hash the current body), so a later edit of this now-handled comment can be detected by the edited-comment re-open rule. Remove its `awaiting_choice` entry, and **mark the comment resolved in the workspace records** — the stacked-mode analogue of a thread resolve, written to `<WS>/` (e.g. the triage/monitoring record), **never** as a mutation on the target PR — exactly as the existing stacked fix-subagent marks findings resolved in `<WS>/triage.json` rather than on the target. Update `review_loop.monitoring.last_activity_at` to reflect that a human comment was just handled (this also keeps the idle tracking correct across cycles).
