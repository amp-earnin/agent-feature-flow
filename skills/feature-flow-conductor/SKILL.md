...
**Edited-comment re-open rule.** ID-based dedupe alone is insufficient: a human may **edit** a comment the loop has already seen, and the edit can change their intent (including changing what they are asking, or changing a selection). This rule covers **both** kinds of already-seen comment that dedupe would otherwise drop: (a) a comment whose `comment_id` is in `handled_comment_ids` (already fixed), **and** (b) a comment tracked by an **open `awaiting_choice` entry** (prompted but not yet answered — its `comment_id` is **not** in `handled_comment_ids` until a fix lands, so case (a) alone would miss it and the dedupe drop would starve a genuinely edited awaiting comment). For each such comment, detect a later edit via a **content hash of the comment body** (or its `updated_at` timestamp) compared against the value recorded when the prompt was posted (for an awaiting entry) or when the comment was handled (for a handled comment). If it changed, **re-open it as unselected** — treat it as a new human comment for this cycle (re-analyze and re-prompt; for an open awaiting entry, replace its prompt with a fresh one carrying a new unique `reply_token_prefix`) rather than silently skipping it on the strength of its ID. An edit is never ignored just because the ID was seen before, whether the comment was already handled or is still awaiting a selection.
