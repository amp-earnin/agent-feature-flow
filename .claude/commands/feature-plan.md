...
3. **Call the `feature-flow-conductor` skill** (via the **Skill tool**) with `TICKET=<ticket>`, `start_stage=plan`, and `mode=only` (or `continue` if the flag was passed). The conductor seeds `state.json` (marking brief complete) before it dispatches the plan subagent.
