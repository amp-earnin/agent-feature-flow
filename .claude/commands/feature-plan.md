...
3. **Invoke the conductor** with `TICKET=<ticket>`, `start_stage=plan`, and `mode=only` (or `continue` if the flag was passed). The conductor will seed `state.json` (marking brief complete) before dispatching the plan subagent.
